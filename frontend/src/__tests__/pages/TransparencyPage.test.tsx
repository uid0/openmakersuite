  ...
  orders: [
    {
      ...
      actual_cost: 200.0,
      cost_per_unit: 100.0,
      item_estimated_cost_today: 180.0,
...
describe('TransparencyPage', () => {
  ...
  it('renders logistics ledger data after loading transparency report', async () => {
    ...
    // "Makersupply Co." also appears multiple times (in orders and ledger
    // sections). It is the ITEM's supplier under both — a reorder request has
    // no supplier of its own — and each surface says so in its label.
    expect(screen.getAllByText('Makersupply Co.').length).toBeGreaterThan(0);
    expect(screen.getByText('Item supplier today:')).toBeInTheDocument();
