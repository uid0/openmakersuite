"""Read helpers for the stored ML demand forecast.

The nightly forecasting task (a later bead) writes one
...
It queries stored rows only -- no ML, no forecasting maths live here. Until the
task populates the table every helper returns ``[]``; that empty state is part
of the contract (and is tested). The urgency ordering mirrors
...
def _urgency_key(forecast: DemandForecast) -> tuple:
    """Sort key: reorder-flagged first, then soonest stockout (nulls last)."""
    return (
        not forecast.needs_reorder,
        forecast.days_until_stockout is None,
        forecast.days_until_stockout if forecast.days_until_stockout is not None else 0.0,
    )
...
def latest_demand_forecasts(*, low_stock_only: bool = False) -> list[DemandForecast]:
    ...
    Args:
        low_stock_only: When ``True``, keep only rows whose ``needs_reorder`` is
            set (available at/below the predictive reorder point).

    Returns:
        A list of :class:`~inventory.models.DemandForecast` rows (possibly
        empty) sorted reorder-flagged-first then soonest-stockout-first.
    """
...
def reorder_alert_forecasts() -> list[DemandForecast]:
    ...
    ``needs_reorder=True`` -- the items a user asked to be pinged about that have
    actually crossed their predictive reorder point.
    """
