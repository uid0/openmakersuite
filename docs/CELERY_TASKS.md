...
| `inventory.tasks.generate_demand_forecasts` | Beat: `crontab(minute=0, hour=4)` — 04:00 UTC daily (`CELERY_BEAT_SCHEDULE["inventory-demand-forecast-nightly"]`) (op-2). Also callable directly for a manual run. | For every active, non-retired, **non-serialized** `InventoryItem`, rebuilds a trailing-365-day daily consumption series (`UsageLog` + shrinkage `StockReconciliation`) and writes **one** new `DemandForecast` row (history retained) via `inventory.services.demand_forecast_engine` — a seasonal Holt-Winters projection (≥60 days history) or a run-rate fallback. Then emits **one** in-app reorder-alert digest notification (per admin, `type=warning`) for opted-in (`reorder_alerts_enabled`) items that crossed their predictive reorder point. Per-item failures are logged and skipped. | None (`@shared_task` defaults) | 30 min global | **Yes** — forecast rows are append-only per run (a re-run just adds a fresh row; the API reads the latest); the digest is deduped per run-date via a `Notification.metadata` marker (`kind`/`run_date`), so a second run the same day sends no duplicate alert. | Re-run manually: `celery -A config call inventory.tasks.generate_demand_forecasts`. One bad item is logged and skipped, not fatal. |
