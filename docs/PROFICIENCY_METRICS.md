...
| Backup restore age                                | Most recent verified restore drill ≤ 90 days                                                | Partial — DB half is **CI-enforced** on every deploy-touching PR (`Prod Stack Smoke` runs `restore-drill.sh --skip-media --skip-smoke` + livez reprobe; `deploy/BACKUP_RESTORE.md` §8.1). Media + config restore still on the manual quarterly K8s cadence (§8.2); R-03 in risk register tracks the residual gap. | Operator log + CI artifacts from `.github/workflows/ci.yml` `AC-25 DB restore drill` step; latest manual drill recorded in `deploy/BACKUP_RESTORE.md` §8.2. |
