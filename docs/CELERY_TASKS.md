...
| `inventory.tasks.update_average_lead_times` | Manual / not currently scheduled | Updates `ItemSupplier.average_lead_time` for items with completed reorders in the last 180 days, labelling each value it writes `average_lead_time_source = measured` | None | 30 min global | **Yes** — recomputes from scratch; idempotent on a stable history | Re-queue from `manage.py shell`. |
| `inventory.tasks.roll_up_meters` | Beat: every 900 s (15 min) — see `CELERY_BEAT_SCHEDULE["inventory-roll-up-meters"]` (EAM bead-1). Also callable directly for a manual rollup. | For each active `AssetMeter`, dispatches to its source adapter (`inventory.services.meter_sources`): `auto_session` folds ended `forgekey.DeviceUsage` sessions after the meter's `rollup_watermark_at` into one runtime-hour `AssetMeterReading` and advances the watermark; `runtime_hours` meters also `F()`-increment `Asset.hours_used` (forecast dual-write). `manual`/`auto_telemetry` are push sources and pull nothing. | None (`@shared_task` defaults) | 30 min global | **Yes** — the per-meter `rollup_watermark_at` makes the session rollup exactly-once (only sessions ended after the watermark are consumed); a re-run with no new ended sessions is a no-op. | Re-run manually: `celery -A config call inventory.tasks.roll_up_meters`, or `run_rollup()` from `manage.py shell`. One bad meter is logged and skipped, not fatal. |
| `inventory.tasks.generate_demand_forecasts` | Beat: `crontab(minute=0, hour=4)` — 04:00 UTC daily (`CELERY_BEAT_SCHEDULE["inventory-demand-forecast-nightly"]`). Also callable directly for a manual run. | For every active, non-retired, **non-serialized** `InventoryItem`, collects the dates it was purchased (`PurchaseOrder.order_date` via its standing, non-voided `PurchaseOrderItem`s, deduped per day; cancelled/voided orders also excluded), averages the gaps between them, and writes **one** new `DemandForecast` row (history retained) via `inventory.services.demand_forecast_engine` — a restock-interval projection of when the item is due to be bought again (`method=restock_interval`, or `insufficient_history` below two purchases). Then emits **one** in-app reorder-alert digest notification (per admin, `type=warning`) for opted-in (`reorder_alerts_enabled`) items due within their supplier lead time — the wait at the supplier we would actually buy from, resolved by `component_forecast.lead_times_for` (see AGENTS.md, "Which supplier's wait the reorder point allows for"). Per-item failures are logged and skipped. | None (`@shared_task` defaults) | 30 min global | **Yes** — forecast rows are append-only per run (a re-run just adds a fresh row; the API reads the latest); the digest is deduped per run-date via a `Notification.metadata` marker (`kind`/`run_date`), so a second run the same day sends no duplicate alert. | Re-run manually: `celery -A config call inventory.tasks.generate_demand_forecasts`. One bad item is logged and skipped, not fatal. |
| `inventory.tasks.snapshot_stock_levels` | Beat: `crontab(minute=0, hour=5, day_of_week=1)` — Monday 05:00 UTC weekly (`CELERY_BEAT_SCHEDULE["inventory-stock-level-snapshot-weekly"]`). Also callable directly for a manual snapshot. | For every active, non-retired `InventoryItem`, `update_or_create`s **one** `StockLevelSnapshot` row keyed on `(item, week-start Monday)` with `count=current_stock`, building the weekly time series the Stock-History chart plots (`stock_history` action). No historical backfill — the series accumulates from the first run. | None (`@shared_task` defaults) | 30 min global | **Yes** — keyed on the week-start date via `update_or_create`, so a re-run within the same week refreshes the existing row's count instead of appending a duplicate point. | Re-run manually: `celery -A config call inventory.tasks.snapshot_stock_levels`. |
...
| `forgekey.tasks.end_idle_device_sessions` | Beat: every 300 s (5 min) — see `CELERY_BEAT_SCHEDULE["forgekey-end-idle-device-sessions"]`. Optional `idle_after_minutes` / `max_session_hours` kwargs override the service defaults (30 min idle window / 12 h wall-clock cap). | Closes idle/runaway access-control `DeviceUsage` sessions (op-vj9): a metered session is idle once its last above-threshold current reading ages past the idle window, a meterless session ends only at the wall-clock cap. Each closed session cuts the asset's primary relay (`disable`) and re-syncs its indicator to available; every close is audited (`session_ended`). | None | 15 min global | **Yes** — already-ended sessions are filtered at the WHERE clause (`ended_at IS NULL`); a re-run only closes sessions still genuinely idle. | Run manually: `celery -A config call forgekey.tasks.end_idle_device_sessions`. |
