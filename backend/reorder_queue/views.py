...
class CanManageWebhooks(BasePermission):
    """
    Permission class for webhook management.

    Allows access if user is:
    - Staff or superuser
    - A SIG/committee chair (manages at least one SIG)
    - Has the 'reorder_queue.manage_webhooks' permission
    """
    ...
    def has_permission(self, request, view):
        ...
        # Staff and superusers can always manage webhooks
        if request.user.is_staff or request.user.is_superuser:
            return True

        # SIG/committee chairs can manage webhooks
        user_managed_sigs = get_user_managed_sigs(request.user)
        if user_managed_sigs.exists():
            return True

        # Check for specific permission
        return request.user.has_perm("reorder_queue.manage_webhooks")
...
class WebHookViewSet(viewsets.ModelViewSet):
    ...
    Allows staff, superusers, SIG/committee chairs, and users with
    'reorder_queue.manage_webhooks' permission to manage webhooks.
    """
...
class PurchasingReportViewSet(viewsets.ViewSet):
    ...
    def lead_time_analysis(self, request):
        ...
        queryset = (
            LeadTimeLog.objects.filter(
                actual_delivery_date__gte=start_date.date())
