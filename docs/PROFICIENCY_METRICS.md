...
| Deployment smoke status                           | `deploy/SMOKE_TESTS.md` runbook executed within 24h of every production deploy              | Partial — runbook exists; CI's `Prod Stack Smoke (prod compose boot)` enforces livez within 60s of gunicorn start (migrations run in their own step, so the budget does not grow with the migration count), but post-deploy operator enforcement is manual | CI smoke job in `.github/workflows/ci.yml`; operator-captured outputs of `deploy/SMOKE_TESTS.md`. |
