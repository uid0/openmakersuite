...
**Scope: ``backend/`` ONLY.** This walk stops at the Python tree. It is not a
theoretical gap: ``frontend/src/pages/ScanPage.tsx``'s reorder form went on
multiplying by a recorded ``0`` after every backend reader had moved onto the
derivation, and review caught it rather than this test. The web half of the
same rule is the ESLint rule ``frontend/eslint-rules/pack-size-single-owner.mjs``,
which counts frontend reads of the column against its own allowlist and fails
``npm run lint`` (and so ``scripts/ci-lint.sh``) on a new one.
