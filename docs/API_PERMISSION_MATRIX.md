...
| GET  | `inventory/items/<id>/metrics/` | public (vendor block withheld) | Computed stock metrics row (QOH/QOO/QA/QC/QIT/RP + case shape). Mirrors `retrieve`'s exposure. **Anonymous:** `lead_time_days`, `unit_cost`, `cost_trend`, `last_po_unit_cost` and the two `supplier_scored_without_*` flags are ABSENT, replaced by `"vendor_data_withheld": true`. `case_size`, `case_size_state` and `is_case_based` stay — a pack size is a shelf fact, not a vendor one (op-c1ke), and the public `current_cases` already depends on it. `case_size_state` says WHICH unknown a null `case_size` is (op-2t4e); it names no vendor and no price, and withholding the cause of a published absence would rebuild that conflation for the anonymous scan flow. **Authenticated:** unchanged, which is why the pinned ScanTTY contract survives. | Not required (read). |
...
| any  | `inventory/item-suppliers/...` (CRUD + `batch/`, `price_history/`, `mark_discontinued/`) | member | `IsAuthenticated` on `ItemSupplierViewSet` (op-anonymous-read-posture, was `IsAuthenticatedOrReadOnly`). A row here IS the vendor relationship: who, their part number, both UPCs, their price and their lead time. |
...
`on_hand_display` / `reorder_display` keys depend on it. `case_size_state` (on
both the item and the metrics payload) follows the key it explains for the same
reason and one more: publishing an absence while withholding its cause is the
op-2t4e conflation rebuilt for anonymous callers, and the state names no vendor
and no price.
...
| Method | Path | Class | Notes |
| --- | --- | --- | --- |
| POST | `reorders/requests/` (create only) | public | `AllowAny` on `ReorderRequestViewSet.create` — required for the QR-scan reorder flow on printed shelf labels. The create input uses `ReorderRequestCreateSerializer`, which accepts only `item`, `quantity`, `requested_by`, `request_notes`, and `priority`; `status` is read-only, so no caller can ask for it. A newly filed anonymous request returns 201 with the serializer's limited fields plus `already_requested: false`. If the item already has a request still `pending`, no second anonymous row is filed and the endpoint returns 200 with the existing request's `id`, `item`, `quantity`, `priority`, and `status`, plus `already_requested: true` and a member-facing `detail`. Admin / cost / invoice / supplier-URL fields are absent from both anonymous response shapes. Authenticated callers additionally pass `can_create_reorder_request`, and a caller who is already an approver for the item (`can_manage_sig_inventory`) has their request stamped `approved` on create (op-tm70); authenticated creates are not deduplicated by the anonymous pending-request rule. |
...
| GET | `reorders/analytics/transparency/` | public (vendor block withheld) | The public spending feed, and it STAYS public — publishing what the makerspace spends is the point of the page. **Anonymous:** the `summary` aggregates (`total_amount_spent`, `total_po_amount_spent`, the counts) are public, and each row keeps its item, category, quantity, status and dates. Those four are now computed over EVERY qualifying row rather than over the capped `orders` / `purchase_orders` arrays (op-transparency-substituted-supplier), so on a space with more than 100 qualifying orders the published total goes UP because it becomes correct — 105 orders worth $1,050.00 used to report as 100 and $1,000.00. The arrays stay capped at 100 / 50. Withheld per row, with `"vendor_data_withheld": true` in their place: `item_supplier_choice`, `item_estimated_cost_today`, `supplier_url`, `invoice_number`, `invoice_url`, `purchase_order_url`, `order_number`, `actual_cost`, `cost_per_unit`, and on the `purchase_orders` block `supplier_name`, `estimated_total` and `actual_total`. **One tuple gates the `orders` and `ledger` rows** (`ORDER_VENDOR_KEYS`), so a key withheld from one cannot be left on the other; `ledger`'s hand-written subset is gone. **`supplier_name`, `estimated_cost` and `cost_variance` are no longer on an order row at all** (op-transparency-substituted-supplier): `ReorderRequest` has no supplier relationship and records no estimate, so all three were resolved from `order.item` at request time and published under the ORDER's name. The item's derivation is served as `item_supplier_choice` (the `SupplierChoiceSerializer` object) and its live quote as `item_estimated_cost_today`; the over/under-budget `cost_variance` has no honest form and was dropped. That detail-shape change does not expose anything new to anonymous readers because every affected key was already withheld. Signed-in readers receive those item-scoped fields plus the order-owned vendor block. `transparency_note` is reworded for each reader rather than left making a claim the payload no longer honours. **`po_number` is deliberately kept** — it is this makerspace's own reference; the vendor's number for the same order is `supplier_order_number`, which this payload has never carried. **`order_number` is deliberately withheld** — `ReorderRequest.order_number` is operator-typed free text with no help text, filed after an order is placed with a vendor, so it holds the vendor's reference at least as often as anything else and an ambiguous field falls closed. **`authentication_classes=[]` was REMOVED from the action**, which is the load-bearing edit: it disabled authentication, so `request.user` was `AnonymousUser` for every caller including a signed-in one and no gate here could have worked. | Not required (read). |
...
| GET | `dashboard/inventory-summary/` | public | The view is `AllowAny`, but the stock VALUATION is gated on the session inside it — a function-based view has no `get_permissions` seam, so the gate is on the field. **Anonymous:** `inventory.{total_items, low_stock_count, items_with_pending_reorders, total_value_withheld, recently_added, low_stock_items}` plus the `assets` counts and `timestamp` — that is the whole body, and no key in it is money or PII. `total_value` and `items_without_price` are ABSENT, replaced by `"total_value_withheld": true`. Absent rather than `null` or `0`: `null` means "no price on file" elsewhere in this payload's family (op-9m2v) and a consumer's `?? 0` would render the withheld figure as a real 0.00; the boolean says the omission is policy, not an empty inventory. `items_without_price` goes with the total because it counts what the total could not value, so it qualifies nothing on its own. **Authenticated:** unchanged — `total_value` (summed through `PriceRollup`) and `items_without_price` beside it. Pinned by `dashboard/tests/test_views.py::test_inventory_summary_withholds_the_valuation_from_an_anonymous_caller` and its logged-in control. |
...
| `quantity_per_package` / `case_size` / `case_size_state` | A pack size is a shelf fact. op-c1ke already answered this for the same column when it excluded `InventoryItem.current_cases` from the supplier derivation, and the public `current_cases` / `on_hand_display` / `reorder_display` keys — which size an anonymous reorder — depend on it. It re-derives nothing once both costs are withheld. `case_size_state` is the CAUSE of that key (op-2t4e) and follows it: `no_orderable_link` says the field is unbuyable, never who is in it. |
