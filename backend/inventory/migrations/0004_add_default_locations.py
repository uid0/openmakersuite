...
def create_default_locations(apps, schema_editor):
    ...
    default_locations = [
        ...
        {"name": "3D Printing Area", "description": "3D printers and filament storage"},
        {"name": "Tool Crib", "description": "Shared hand tools and small equipment"},
        {
            "name": "Storage Room",
    ...
    for loc_data in default_locations:
        Location.objects.get_or_create(
            name=loc_data["name"], defaults={"description": loc_data["description"]}
        )
...
def remove_default_locations(apps, schema_editor):
    ...
    Location.objects.filter(
        name__in=default_names, description__startswith="Primary workspace"
    ).delete() | Location.objects.filter(
        name__in=default_names, description__startswith="ESD-safe workspace"
    ).delete() | Location.objects.filter(
        name__in=default_names, description__startswith="Woodworking area"
    ).delete() | Location.objects.filter(
        name__in=default_names, description__startswith="Metal fabrication"
    ).delete() | Location.objects.filter(
        name__in=default_names, description__startswith="3D printers"
    ).delete() | Location.objects.filter(
        name__in=default_names, description__startswith="Shared hand tools"
    ).delete() | Location.objects.filter(
        name__in=default_names, description__startswith="General supplies"
    ).delete() | Location.objects.filter(
        name__in=default_names, description__startswith="Paper, pens"
    ).delete() | Location.objects.filter(
        name__in=default_names, description__startswith="PPE, first aid"
    ).delete() | Location.objects.filter(
        name__in=default_names, description__startswith="Sandpaper, shop rags"
    ).delete()
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(
            create_default_locations, reverse_code=remove_default_locations
        ),
    ]
