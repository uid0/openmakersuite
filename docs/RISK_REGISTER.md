...
| R-04 | Critical | Low | Device control or lockout workflows fail silently. | ForgeKey, MQTT, Celery, EMQX | Facilities/admin | Task monitoring, device status tests, operator alerts | Mitigated | Failed/retrying tasks visible via `django_celery_results.TaskResult` admin (`backend/config/celery_admin.py`, `test_celery_admin.py`); `RedactingDatabaseBackend` scrubs secrets out of stored tracebacks/results; webhook delivery rows carry `success_count`/`failure_count`/`last_error` (`/admin/reorder_queue/webhook/`); Flower live worker view documented in `deploy/COMPOSE_RUNBOOK.md` §10. MQTT-side recovery covered by mqtt_consumer reconnect loop + `@sentry_sdk.crons.monitor` on scheduled tasks (`forgekey.mark_stale_devices_offline`, `forgekey.prune_device_photos`). gh #458. |
| R-05 | High | Medium | Celery tasks fail, retry forever, or duplicate external side effects. | Reorders, webhooks, donations, vendors, ForgeKey, location check-ins | Maintainer/operator | Task result dashboard, retry/idempotency tests | Mitigated | Per-task inventory enforced by `backend/config/tests/test_celery_task_inventory.py::test_every_registered_task_is_documented` (drift gate against `docs/CELERY_TASKS.md`); beat schedule pinned by `test_celery_schedule.py` (name/dotted-path/cadence + registered-task resolution); idempotency expectations covered by `test_task_idempotency.py` for every external-side-effect task — image-download replay-skip, webhook payload dedup-id, MQTT level-trigger, vendor clean-run, donor no-retry config, location-checkin stable id. Non-replay-safe task (`donations.send_quarterly_donor_updates`) carries explicit per-donor recovery path in `docs/CELERY_TASKS.md` §Replay paths. gh #458. |
...
| R-10 | Medium | Medium | Users cannot complete workflows on phones, kiosks, or low-connectivity networks. | Frontend | Product maintainer | Playwright mobile tests, manual smoke checklist | Mitigated | AC-14 (camera-free code entry) covered by `frontend/e2e/code-entry-fallback.spec.ts`; AC-15 (clear final-state surfaces) covered by `ThanksPage` / `FixtureScanPage` / `MakerBoxScanPage` / `TaxReceiptLookupPage` unit suites plus `frontend/e2e/scan-mobile.spec.ts` and `frontend/e2e/invite-redeem-mobile.spec.ts`; AC-16 (kiosk / shared-device isolation) covered by `frontend/e2e/kiosk-session-isolation.spec.ts`. Low-connectivity friction is reduced by notifications ETag/304 (#596) and the responsive Inventory Overview / Universal Scanner / Invite Redeem surfaces (#588, #590, #606). |
| R-11 | Medium | Low | Observability captures too much PII or too little operational context. | Sentry, logs, API, Celery | Maintainer/operator | Sentry/log review, privacy checklist | Mitigated | `config.observability_redaction.redact` plus rollout to all four named surfaces (gh #333, #378): DRF error envelopes, webhook `last_error`, Celery `TaskResult.{traceback,result}` via `RedactingDatabaseBackend`, and ForgeKey MQTT consumer (`OccupancyEvent.raw_payload`, OTA `error_message`). |
