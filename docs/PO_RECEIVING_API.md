...
| `POST /api/reorders/receipts/scan_barcode/` | A separate, older inline receive path keyed on UPC. Refuses kit lines and writes no audit event. Prefer `receive` with a `scan_codes` match. |
