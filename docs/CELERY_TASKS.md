...
| `inventory.tasks.generate_demand_forecasts` | Beat: `crontab(minute=0, hour=4)` — 04:00 UTC daily (`CELERY_BEAT_SCHEDULE["inventory-demand-forecast-nightly"]`). Also callable directly for a manual run. | For every active, non-retired, **non-serialized** `InventoryItem`, collects the dates it was purchased (`PurchaseOrder.order_date` via its `PurchaseOrderItem`s, deduped per day, cancelled/voided orders excluded), averages the gaps between them, and writes **one** new `DemandForecast` row (history retained) via `inventory.services.demand_forecast_engine` — a restock-interval projection of when the item is due to be bought again (`method=restock_interval`, or `insufficient_history` below two purchases). Then emits **one** in-app reorder-alert digest notification (per admin, `type=warning`) for opted-in (`reorder_alerts_enabled`) items due within their supplier lead time — the wait at the supplier we would actually buy from, resolved by `component_forecast.lead_times_for` (see AGENTS.md, "Which supplier's wait the reorder point allows for"). Per-item failures are logged and skipped. | None (`@shared_task` defaults) | 30 min global | **Yes** — forecast rows are append-only per run (a re-run just adds a fresh row; the API reads the latest); the digest is deduped per run-date via a `Notification.metadata` marker (`kind`/`run_date`), so a second run the same day sends no duplicate alert. | Re-run manually: `celery -A config call inventory.tasks.generate_demand_forecasts`. One bad item is logged and skipped, not fatal. |
