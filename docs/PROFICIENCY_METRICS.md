...
| Maintenance work order review                              | unit + e2e | `frontend/src/__tests__/pages/WorkOrderPage.test.tsx`, `ThirdPartyWorkOrderPage.test.tsx`, `MaintenanceDashboard.test.tsx`; e2e in `frontend/e2e/maintenance-work-order.spec.ts`. |
| Logistics dashboard                                        | unit       | `frontend/src/__tests__/pages/LogisticsDashboard.test.tsx`, `DashboardPage.test.tsx`.                        |
| Kiosk display                                              | unit + manual | `frontend/src/__tests__/pages/KioskDisplayPage.test.tsx`, `TVDashboard.test.tsx`, `ScreenEditPage.test.tsx`; physical-display hardware verification per `docs/FRONTEND_JOURNEY_INVENTORY.md`. |
...
| Settings + webhooks                                        | unit       | `frontend/src/__tests__/pages/UserProfilePage.test.tsx`, `SiteSettingsPage.test.tsx`, `WebhookListPage.test.tsx`, `WebhookFormPage.test.tsx`, `WebhookDetailPage.test.tsx`. |
...
| Production smoke checks (AC-35)                            | CI + manual| `.github/workflows/ci.yml` `Prod Stack Smoke (livez within 120s)` job + manual runbook in `deploy/SMOKE_TESTS.md`. |
...
| ------------------------------------------------- | ---------------- | ---------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| Critical-path e2e coverage                        | 100% of `docs/FRONTEND_JOURNEY_INVENTORY.md` rows have a Playwright spec | Partial — `frontend/e2e/` covers public-to-staff loop UI-complete (`public-to-staff.spec.ts`), inventory browse + search (`inventory-browse.spec.ts`), mobile code-entry fallback (`code-entry-fallback.spec.ts`), work-order review (`maintenance-work-order.spec.ts`), asset-scan, admin-dashboard-assets, and SIG dashboard; reorder triage and ForgeKey e2e remain gaps | Run `frontend/e2e` and cross-reference inventory.                          |
...
| Backup restore age                                | Most recent verified restore drill ≤ 90 days                                                | Mitigated for DB + media + Compose config — all three halves are **CI-enforced** on every deploy-touching PR. DB via `Prod Stack Smoke` running `restore-drill.sh --skip-media --skip-smoke` + livez reprobe (`deploy/BACKUP_RESTORE.md` §8.1). Media via the seed→backup→wipe→restore→verify fixture step added 2026-06-09 (gh-454). Compose config (.env + override + EMQX bootstrap) via the mutate→backup→corrupt→restore→hash-verify step also added 2026-06-09 (gh-454) using the new `scripts/restore-config.sh`. The k8s ConfigMap / Secret restore for the DR-namespace shape stays on the manual quarterly cadence in §8.2; R-03 in risk register tracks that residual cross-namespace gap. | Operator log + CI artifacts from `.github/workflows/ci.yml` `AC-25 DB restore drill`, `AC-25 media restore drill`, and `AC-25 config restore drill` steps; latest manual k8s drill recorded in `deploy/BACKUP_RESTORE.md` §8.2. |
...
| Deployment smoke status                           | `deploy/SMOKE_TESTS.md` runbook executed within 24h of every production deploy              | Partial — runbook exists; CI's `Prod Stack Smoke (livez within 120s)` enforces livez within 120s, but post-deploy operator enforcement is manual | CI smoke job in `.github/workflows/ci.yml`; operator-captured outputs of `deploy/SMOKE_TESTS.md`. |
