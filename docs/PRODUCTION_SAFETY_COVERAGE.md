...
| Cookie security overrides | Warns when `SESSION_COOKIE_SECURE` or `CSRF_COOKIE_SECURE` is explicitly set to a case-insensitive falsy value (`0`, `false`, `off`, `no`, or empty) while `DEBUG=0`. | Mitigated | `scripts/validate-prod-env.sh:292-313` | `test_cookie_secure_falsy_override_warns`, `test_cookie_secure_non_falsy_override_has_no_warning`, `test_cookie_secure_unset_has_no_warning`. |
...
| `POST /api/reorders/requests/` (create only) | `AllowAny` create with response field allow-lists. | `backend/reorder_queue/views.py` `ReorderRequestViewSet.create` uses the limited create serializer for new rows and an explicit allow-list for an existing pending request; admin / cost / supplier fields are not exposed. Anonymous duplicates are also suppressed per item while a request is pending. See the authoritative response contract in [API_PERMISSION_MATRIX.md](API_PERMISSION_MATRIX.md). | Mitigated by field allow-lists and pending-request deduplication; no throttle. |
| `POST /api/location-checkins/checkins/checkin/` | Per-IP throttle. | `backend/location_checkins/views.py:62` (`@action ... permission_classes=[AllowAny]`); no throttle. | Gap |
| `POST /api/location-checkins/checkins/` (kiosk create) | Per-IP throttle. | `backend/location_checkins/views.py:46-60` (`get_permissions()` returns `[AllowAny()]` for `create`); no throttle. | Gap |
