...
| Backup restore age                                | Most recent verified restore drill ≤ 90 days                                                | Mitigated for DB + media — both halves are **CI-enforced** on every deploy-touching PR. DB via `Prod Stack Smoke` running `restore-drill.sh --skip-media --skip-smoke` + livez reprobe (`deploy/BACKUP_RESTORE.md` §8.1). Media via the seed→backup→wipe→restore→verify fixture step added 2026-06-09 (gh-454). Config restore (the k8s ConfigMap / Secret tree) still on the manual quarterly cadence in §8.2; R-03 in risk register tracks that residual gap. | Operator log + CI artifacts from `.github/workflows/ci.yml` `AC-25 DB restore drill` + `AC-25 media restore drill` steps; latest manual config drill recorded in `deploy/BACKUP_RESTORE.md` §8.2. |
