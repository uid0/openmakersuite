...
| View / approve / receive / add lines to purchase order | `/purchasing/orders/:orderId` (`PurchaseOrderPage.tsx`) | staff/admin | `mark-delivered`, `receive` (per-line-item), `mark-received`, `reopen-short`, and the add-a-line entry patch the page from the response (no reload through the loading placeholder); the add-a-line field takes a typed or scanned identifier and its Enter submits, so a scanner needs no mouse, and the control is absent (not disabled) on any non-draft order, with an "Add a custom line" disclosure beneath it for a freeform line the catalogue does not carry; each line offers exactly one removal action, chosen off the order's `can_delete_items` and never re-derived from `status` — "Delete Line" while the order is pre-send (destroys the line, takes no reason, confirms by naming the line, quantity, unit price and total, and patches the page from the response) or "Void Item" once the supplier holds a copy (reason required, line struck off and kept on the record), with neither offered on a line that records a receipt, where a pre-send order says so in the row rather than leaving an unexplained gap; the receive panel scans a tracking barcode and carrier, records the quantity that actually arrived (a short or over receipt is flagged on the line, never rounded to the ordered figure), offers a close-short checkbox with a reason when the receipt falls short, and captures serials with optional lot and expiry against the identities the line credits — the components on a kit line, never the kit; units stocked with no serial recorded stay visible as `serials_outstanding`. Endpoint semantics are owned by [`PO_RECEIVING_API.md`](PO_RECEIVING_API.md); when the order's status cannot receive, the page says why instead of silently dropping the receive buttons; "Send to Supplier" is offered from `status` but disabled with the server's own sentence printed beside it as visible text rather than only as a hover tooltip whenever the order's `send_blocked_reason` is non-null, read off that field rather than counted from the lines here so the button and the endpoint cannot disagree; receipt action gated by permission; auth expiration returns to PO |
